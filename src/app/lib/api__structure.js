...
export async function fetchSessionByDeviceId(deviceId) {
  try {
    ...
    return { success: true, output: existingSession }
  } catch (error) {
    handleError(error);
  }
}
...
export async function addSession(sessionData) {
  try {
    ...
    const deviceStatusUpdate = await editInventoryById(sessionData.device_id, { status: 'booked' });
    ...
    const result = await pb.collection('sessions').create(newSession);
    console.log(result);
  } catch (error) {
    handleError(error);
  }
...
export async function extendSession({ id, duration, no_of_players, branchId }) {
  try {
    const pricing = await fetchPricings({ branch_id: branchId });
    console.log(pricing);
    let total_price;
    let credit_amt;
    switch (no_of_players) {
      case 1:
        total_price = pricing.items[0].single_player * duration;
        break;
      case 2:
        total_price = pricing.items[0].multi_player * no_of_players * duration
        break;
      default:
        total_price = pricing.items[0].over_three_player * no_of_players * duration
        break;
    }
    credit_amt = total_price * pricing.items[0].credit_rate * 0.01;
    return await pb.collection('sessions').update(id, {
      duration_hours: duration,
      no_of_players: no_of_players,
      total_amount: total_price,
      reward_points_earned: credit_amt.toFixed(0)
    });
  } catch (error) {
    handleError(error);
  }
...
export async function endSession({ sessionId, AmountPayable, RedeemAmount = 0, PaymentMode }) {
  try {
    const session = await pb.collection('sessions').getOne(sessionId);
    const customerId = session?.customer_id;
    const deviceId = session?.device_id
    const customerInfo = await fetchCustomerById({ customerId: customerId });
    const credit_amt = session?.reward_points_earned || 0;
    const remainingPoints = customerInfo.total_rewards - RedeemAmount + credit_amt;
    await editCrmById(customerId, {
      total_rewards: remainingPoints
    });

    // Calculate actual duration
    const startTime = new Date(session.session_in);
    const endTime = new Date(startTime.getHours() + session.duration_hours);

    await pb.collection('sessions').update(sessionId, {
      session_out: endTime.toISOString(),
      status: 'closed',
      reward_points_used: RedeemAmount,
      amount_paid: AmountPayable,
      payment_mode: PaymentMode
    });

    return await editInventoryById(deviceId, {
      status: 'open'
    })
  } catch (error) {
    handleError(error);
...
export async function fetchCustomerById({ customerId }) {
  try {
    const response = await pb.collection('customers').getOne(customerId);
    return response; // Return the list of customers
  } catch (error) {
    console.error('Error fetching customers:', error);
    throw error; // Handle error appropriately
  }
...
export async function fetchCustomerId({ customerName, customerContact, branchId, userId }) {
  try {
    const response = await pb.collection('customers').getList(1, 50, {
      filter: pb.filter('customer_name = {:customerName} && customer_contact ={:customerContact}', {
        customerName,
        customerContact
      })
    });
    if (response?.items.length > 0) {
      ...
      const result = await pb.collection('customers').create(newCustomer);
      return result.id;
    }
  } catch (error) {
    console.error('Error fetching customers:', error);
